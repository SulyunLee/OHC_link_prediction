...
Description:
    ...
    We use the following supervised approach: neural network
...
if __name__ == "__main__":
    ...
    for i, week in enumerate(tqdm(range(startweek, endweek))):
        ...
        # Normalize the features (minimax 0 - 1)
        test_features_df = (test_features_df-train_features_df.min()) / (train_features_df.max()-train_features_df.min())
        ...
        # Make the labels into one-hot vector
        num_classes = 2
        y_onehot_train = to_categorical(y_train, num_classes)
        y_onehot_test = to_categorical(y_test, num_classes)
        ...
        try:
            ...
            for idx, k in enumerate(k_list):
                topk_results[i, idx*2: idx*2+2] = evaluate_topk(y_test, y_pred_prob, model, k=k)
    ...
    df_precision_test.to_csv(write_folder / "precision_nn_week_{}_{}.csv".format(startweek, endweek))
    df_recall_test.to_csv(write_folder / "recall_nn_week_{}_{}.csv".format(startweek, endweek))
    ...
    for idx, k in enumerate(k_list):
        ...
        df.to_csv(write_folder / "top{}_precision_nn_week_{}_{}.csv".format(k, startweek, endweek))
        # ndcg
        df = pd.DataFrame(
        ...
        df.to_csv(write_folder / "top{}_ndcg_nn_week_{}_{}.csv".format(k, startweek, endweek))
